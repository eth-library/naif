...
dict(icon="database", color="warning", value=pct(any_tobi, total))
```

```{python}
#| content: valuebox
#| title: "OpenAlex Findable"
dict(icon="search", color="info", value=pct(openalex_findable, total))
```

```{python}
#| content: valuebox
#| title: "Dimensions Findable"
dict(icon="diagram-3", color="primary", value=pct(dimensions_findable, total))
```
